...
def run_rep_briefing(rep_id: str, reference_date: date, targeting_plan_path: str | None = None) -> dict:
    ...
    assist_actions = _get_rep_assist_growers(territory_id, targeting_plan_path)
...
def _get_rep_assist_growers(territory_id: str, targeting_plan_path: str | None = None) -> list:
    plan_path = targeting_plan_path or TARGETING_PLAN_PATH
...
def _gen_briefing(
    ...
    def _visit_label(days: int) -> str:
        return "never visited" if days >= 999 else f"{days} days since last visit"

    action_lines = "\n".join(
        f"{a['rank']}. [{a['type']}] "
        + (f"Visit {a.get('tehsil')} — {_visit_label(a.get('days_since_visit', 0))}" if a["type"] == "visit_gap"
...
and remind them about the {offline_count} offline growers who can only be reached in person."""
    ...
    try:
        ...
        if not top:
            return (
                f"No urgent actions were detected for {rep_id} on {reference_date}. "
                f"Use this week to maintain retailer relationships and cover offline growers ({offline_count}) "
                "with proactive check-ins."
            )
        if top["type"] == "restock_alert":
            first_action = f"restock {top.get('sku')} (~{top.get('weeks_until_oos')} weeks to OOS)"
        elif top["type"] == "visit_gap":
            _d = top.get("days_since_visit", 0)
            _dl = "never visited" if _d >= 999 else f"{_d} days since visit"
            first_action = f"visit {top.get('tehsil')} ({_dl})"
        else:
            first_action = f"run assisted campaign for {top.get('crop')} ({top.get('grower_count')} growers)"
        return (
            f"Priority for {rep_id} on {reference_date}: {first_action}. "
            f"Execute highest-ranked actions first and ensure offline-grower follow-up for {offline_count} growers "
            "who need in-person outreach."
        )
